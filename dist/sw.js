...
function se(s) {
  if (!s) throw new h('add-to-cache-list-unexpected-type', { entry: s });
  if (typeof s == 'string') {
    const r = new URL(s, location.href);
    return { cacheKey: r.href, url: r.href };
  }
  const { revision: e, url: t } = s;
  if (!t) throw new h('add-to-cache-list-unexpected-type', { entry: s });
  if (!e) {
  ...
  return n.searchParams.set(te, e), { cacheKey: n.href, url: a.href };
}
class ne {
  constructor() {
    (this.updatedURLs = []),
...
class ue {
  ...
  async cachePut(e, t) {
    ...
    if (!t) throw new h('cache-put-with-no-response', { url: ce(a.url) });
    ...
    const { cacheName: i, matchOptions: c } = this._strategy,
      ...
      m = l ? await oe(o, a.clone(), ['__WB_REVISION__'], c) : null;
    try {
...
class k {
  ...
  handleAll(e) {
    ...
    const t = e.event,
      ...
      r = new ue(this, { event: t, request: n, params: a }),
...
class p extends k {
  constructor(e = {}) {
    (e.cacheName = R.getPrecacheName(e.cacheName)),
      super(e),
  ...
  constructor({
    ...
    (this._urlsToCacheKeys = new Map()),
      ...
      (this._strategy = new p({
        ...
        plugins: [...t, new ae({ precacheController: this })],
        fallbackToNetwork: n,
      })),
    ...
    for (const n of e) {
      ...
      if (typeof n != 'string' && n.integrity) {
        if (
          ...
          throw new h('add-to-cache-list-conflicting-integrities', { url: r });
...
let D;
const M = () => (D || (D = new de()), D);
try {
  self['workbox:routing:6.5.3'] && _();
} catch {}
const $ = 'GET',
  x = (s) => (s && typeof s == 'object' ? s : { handle: s });
class b {
  constructor(e, t, n = $) {
...
let w;
const A = () => (
  w || ((w = new pe()), w.addFetchListener(), w.addCacheListener()), w
);
function S(s, e, t) {
  let n;
  if (typeof s == 'string') {
    ...
    n = new b(i, e, t);
  } else if (s instanceof RegExp) n = new fe(s, e, t);
  else if (typeof s == 'function') n = new b(s, e, t);
  else if (s instanceof b) n = s;
  else
...
const _e = '-precache-',
  be = async (s, e = _e) => {
    const n = (await self.caches.keys()).filter(
      (a) => a.includes(e) && a.includes(self.registration.scope) && a !== s
    );
...
function Re() {
  self.addEventListener('activate', (s) => {
    ...
    s.waitUntil(be(e).then((t) => {}));
  });
}
function L(s) {
  return M().matchPrecache(s);
}
function Ce(s) {
  M().precache(s);
}
...
class Ee extends k {
  ...
  async _handle(e, t) {
    ...
    if (!a) throw new h('no-response', { url: e.url, error: r });
    return a;
  }
}
class De extends k {
...
function Pe() {
  return (
    F ||
    (F = [IDBDatabase, IDBObjectStore, IDBIndex, IDBCursor, IDBTransaction])
  );
}
function Ne() {
  return (
...
function Ie(s) {
  ...
  return (
    e
      .then((t) => {
        t instanceof IDBCursor && z.set(t, s);
      })
      .catch(() => {}),
    K.set(e, s),
    e
...
function Se(s) {
  return s === IDBDatabase.prototype.transaction &&
    ...
    : Ne().includes(s)
    ? function (...e) {
        return s.apply(T(this), e), f(z.get(this));
      }
    : function (...e) {
...
function Ke(s) {
  return typeof s == 'function'
    ? Se(s)
    : (s instanceof IDBTransaction && Me(s), Te(s, Pe()) ? new Proxy(s, I) : s);
}
function f(s) {
  if (s instanceof IDBRequest) return Ie(s);
  if (U.has(s)) return U.get(s);
  const e = Ke(s);
  return e !== s && (U.set(s, e), K.set(e, s)), e;
}
const T = (s) => K.get(s);
function Oe(s, e, { blocked: t, upgrade: n, blocking: a, terminated: r } = {}) {
  const i = indexedDB.open(s, e),
    c = f(i);
  return (
    n &&
      i.addEventListener('upgradeneeded', (o) => {
        n(f(i.result), o.oldVersion, o.newVersion, f(i.transaction));
      }),
    t && i.addEventListener('blocked', () => t()),
    c
      .then((o) => {
        r && o.addEventListener('close', () => r()),
          a && o.addEventListener('versionchange', () => a());
      })
      .catch(() => {}),
    c
  );
...
function ve(s, { blocked: e } = {}) {
  ...
  return e && t.addEventListener('blocked', () => e()), f(t).then(() => {});
}
...
function j(s, e) {
  ...
  const r = async function (i, ...c) {
    ...
    return (
      n && (l = l.index(c.shift())),
      (await Promise.all([l[t](...c), a && o.done]))[0]
    );
  };
  return P.set(e, r), r;
}
...
class qe {
  constructor(e = {}) {
    (this.cachedResponseWillBeUsed = async ({
      ...
      e.purgeOnQuotaError && Ue(() => this.deleteCacheAndMetadata());
  }
  _getCacheExpiration(e) {
    if (e === R.getRuntimeName()) throw new h('expire-custom-caches-only');
    let t = this._cacheExpirations.get(e);
    return (
      t || ((t = new He(e, this._config)), this._cacheExpirations.set(e, t)), t
    );
  }
...
function X(s) {
  self.addEventListener('install', (e) => {
    const t = s.urls.map(
      (n) => s.strategy.handleAll({ event: e, request: new Request(n) })[1]
    );
    e.waitUntil(Promise.all(t));
  });
}
function Ve(s = {}) {
  const e = ({ request: o }) => o.destination === 'image',
    t = s.cacheName || 'images',
    n = s.matchCallback || e,
    a = s.maxAgeSeconds || 30 * 24 * 60 * 60,
    r = s.maxEntries || 60,
    i = s.plugins || [];
  i.push(new G({ statuses: [0, 200] })),
    i.push(new qe({ maxEntries: r, maxAgeSeconds: a }));
  const c = new De({ cacheName: t, plugins: i });
  S(n, c), s.warmCache && X({ urls: s.warmCache, strategy: c });
}
function $e(s = {}) {
  const e = ({ request: i }) =>
      i.destination === 'style' ||
      i.destination === 'script' ||
      i.destination === 'worker',
    t = s.cacheName || 'static-resources',
    n = s.matchCallback || e,
    a = s.plugins || [];
  a.push(new G({ statuses: [0, 200] }));
  const r = new Ee({ cacheName: t, plugins: a });
  S(n, r), s.warmCache && X({ urls: s.warmCache, strategy: r });
}
function Ge(s) {
  A().setCatchHandler(s);
}
function Qe(s = {}) {
  ...
  self.addEventListener('install', (r) => {
    ...
    t && i.push(t),
      n && i.push(n),
      r.waitUntil(
        self.caches.open('workbox-offline-fallbacks').then((c) => c.addAll(i))
      );
  }),
    Ge(async (r) => {
      const i = r.request.destination,
        c = await self.caches.open('workbox-offline-fallbacks');
      return i === 'document'
        ? (await L(e)) || (await c.match(e)) || Response.error()
        : i === 'image' && t !== !1
        ? (await L(t)) || (await c.match(t)) || Response.error()
        : (i === 'font' && n !== !1 && ((await L(n)) || (await c.match(n)))) ||
          Response.error();
    });
}
function ze(s) {
  ...
  async _handle(e, t) {
    ...
    try {
      ...
      if (this._networkTimeoutSeconds) {
        const i = V(this._networkTimeoutSeconds * 1e3);
        r.push(i);
      }
    ...
    if (!a) throw new h('no-response', { url: e.url, error: n });
    return a;
  }
}
Re();
ze(new Je());
xe([
  { revision: null, url: 'assets/index.fdaa9688.js' },
  { revision: 'f6fcea47664fd12088de1672acded33a', url: 'css/styles.css' },
  { revision: 'bddbbc64ffaca728a64c0523a4851126', url: 'index.html' },
  ...
  { revision: '2c63d81846c4043361b75569658c717e', url: 'manifest.webmanifest' },
]);
$e();
Ve();
Qe({ pageFallback: '/offline.html' });
